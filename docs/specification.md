...
2. 一意性
   - アクティビティタイプごとに異なるGUIDを生成
   - 同じアクティビティタイプでも異なるGUIDを割り当て
   - GUIDの重複を防ぐためのチェック機能を実装
   - 既にGUIDが付与されている場合はスキップ

3. アクティビティタイプ別のGUID管理
   - アクティビティタイプごとにGUIDを管理
   - 同じアクティビティタイプ内でのGUID重複を防止
   - ログにアクティビティタイプとGUIDの対応を記録
